...
public class AssetReleaseHandle : MonoBehaviour
{
    ...
    public static void Add(AssetHandle assetHandle, GameObject target)
    {
        if (target == null)
        {
            GameObject SceneRoot = GameEntry.Pool.GameObjectPool.GameObjectPoolRoot;
            target = SceneRoot;
            GameEntry.Log(ELogCategory.Loader, string.Format("因为{0}没有可绑定的target， 所以绑定到了{1}上， 随当前场景销毁而减少引用计数", assetHandle.AssetObject, SceneRoot));
        }
