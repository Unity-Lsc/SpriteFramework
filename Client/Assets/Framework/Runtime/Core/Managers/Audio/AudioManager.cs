...
namespace SpriteFramework
{
    ...
    public class AudioManager
    {
        ...
        public AudioManager()
        {
            m_AudioRoutinePrefab = new GameObject("AudioSource", typeof(AudioRoutine), typeof(GameObjectDespawnHandle), typeof(AudioSource)).GetComponent<AudioRoutine>();
            m_AudioRoutinePrefab.Init();
            m_AudioRoutinePrefab.transform.SetParent(GameEntry.Instance.transform);
            m_AudioRoutinePrefab.AudioSource.playOnAwake = false;
            m_AudioRoutinePrefab.AudioSource.maxDistance = 20;
            m_AudioRoutinePrefab.AudioSource.outputAudioMixerGroup = GameEntry.Instance.MasterMixer.FindMatchingGroups("Audio")[0];
        ...
        /// 按资源路径播放BGM
        /// </summary>
        public async void PlayBGM(string assetFullPath, BGMPlayOptions options = null)
        {
            if (string.IsNullOrEmpty(assetFullPath))
            {
                GameEntry.LogError(ELogCategory.Audio, "PlayBGM assetFullPath is null.");
                return;
            }

            options ??= new BGMPlayOptions();

            var operation = GameEntry.Loader.DefaultPackage.LoadAssetAsync(assetFullPath);
            await operation.Task;
            PlayBGM(operation.AssetObject as AudioClip, options, operation);
        }

        /// <summary>
        /// 按Clip资源播放BGM
        /// </summary>
        public void PlayBGM(AudioClip audioClip, BGMPlayOptions options, AssetHandle assetHandle)
        {
            if (audioClip == null)
            {
                GameEntry.LogError(ELogCategory.Audio, "audioClip==null");
                return;
            }
            ...
            targetBGMIsLoop = options.IsLoop;
            targetBGMClipVolume = options.Volume;
            isFadeIn = options.IsFadeIn;
            isFadeOut = options.IsFadeOut;
            ...
            GameEntry.Log(ELogCategory.Audio, $"PlayBGM, audioClip={audioClip}, volume={options.Volume}");
        }
        ...
        public void StopBGM(bool fadeOut)
        {
            ...
            GameEntry.Log(ELogCategory.Audio, "StopBGM");
        }
        ...
        public void PauseBGM(bool isPause)
        {
            ...
            GameEntry.Log(ELogCategory.Audio, BGMSource.clip + "PauseBGM");
        ...
        private AudioRoutine m_AudioRoutinePrefab;

        /// <summary>
        /// 按资源路径播放3D音效
        /// </summary>
        public async void PlayAudio(string assetFullPath, Vector3 point, AudioPlayOptions options = null)
        {
            if (string.IsNullOrEmpty(assetFullPath))
            {
                GameEntry.LogError(ELogCategory.Audio, "PlayAudio assetFullPath is null.");
                return;
            }
            ...
            AudioRoutine routine = CreateAudioRoutine(operation.AssetObject as AudioClip, options.Volume, options.Priority);
            routine.AudioSource.spatialBlend = 1;
            routine.transform.position = point;
            routine.AutoDespawnHandle.OnDespawn += () => operation.Release();
        }

        /// <summary>
        /// 按资源路径播放2D音效
        /// </summary>
        public async void PlayAudio(string assetFullPath, AudioPlayOptions options = null)
        {
            if (string.IsNullOrEmpty(assetFullPath))
            {
                GameEntry.LogError(ELogCategory.Audio, "PlayAudio assetFullPath is null.");
        ...
        {
            AudioRoutine routine = GameEntry.Pool.GameObjectPool.Spawn(m_AudioRoutinePrefab.gameObject, SpawnPoolId.Audio).GetComponent<AudioRoutine>();
