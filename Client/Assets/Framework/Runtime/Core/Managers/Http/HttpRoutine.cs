...
namespace SpriteFramework
{
    ...
    public class HttpRoutine
    {
        ...
        {
            GameEntry.Log(ELogCategory.NetWork, string.Format("Get请求:{0}, {1}次重试", this.url, currRetry));
        ...
        private void PostUrl(string url)
        {
            ...
            GameEntry.Log(ELogCategory.NetWork, $"Post请求:{this.url}, {currRetry}次重试==>>{json}");
        ...
        private IEnumerator Request(UnityWebRequest data)
        {
            ...
            {
                GameEntry.Log(ELogCategory.NetWork, string.Format("WebAPI回调:{0}, ==>>{1}", url, data.downloadHandler.text));
            }
